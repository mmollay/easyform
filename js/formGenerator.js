  ...
  form.form({
    ...
    onSuccess: function (event, fields) {
      ...
      if (!isSubmitting) {
        ...
        var isValid =
          typeof validateCKEditors === "function"
            ? validateCKEditors(formId, formValidationRules)
            : true;
        if (isValid) {
          isSubmitting = true;
          ...
          submitForm(formId, responseType, successFunction, function () {
            isSubmitting = false;
          });
        }
      }
...
function initializeDropdowns(form) {
  ...
  if (!form || !form.length) {
    // Kein Warn-Log - normales Verhalten bei Seiten ohne Formulare
    return;
  }
  ...
  form.find(".ui.dropdown").each(function () {
    ...
    if (onChangeAttr) {
      settings.onChange = function (value, text, $selected) {
        try {
          // Sichere Alternative: Funktion aus window-Scope aufrufen
          // Unterstützt sowohl Funktionsnamen als auch Pfade wie "namespace.function"
          var parts = onChangeAttr.split(".");
          var fn = window;
          for (var i = 0; i < parts.length; i++) {
            fn = fn[parts[i]];
            if (typeof fn === "undefined") {
              console.warn("onChange Funktion nicht gefunden:", onChangeAttr);
              return;
            }
          }
          if (typeof fn === "function") {
            fn(value, text, $selected);
          } else {
            console.warn("onChange ist keine Funktion:", onChangeAttr);
          }
        } catch (error) {
...
function showToast(message, type) {
  ...
  messageEl.innerHTML =
    '<i class="close icon"></i><div class="content"><div class="header">' +
    message +
    "</div></div>";
