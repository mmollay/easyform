...
function loadListGenerator(url, customState = {}) {
  ...
  const savedState =
    customState.saveState !== false
      ? JSON.parse(localStorage.getItem(`${url}_${contentId}`)) || {}
      : {};
  ...
  // Add loading class for subtle opacity change instead of replacing content
  $(`#${contentId}`).addClass("loading-content");
  ...
  $.ajax({
    ...
    success: function (response) {
      $(`#${contentId}`).removeClass("loading-content").html(response);
    ...
    error: function (xhr, status, error) {
      ...
      console.error(
        `AJAX-Anfrage fehlgeschlagen für ${contentId}:`,
        status,
        error,
      );
    },
...
function reloadTable(contentId = null) {
  ...
  if (!targetContentId) {
    console.error(
      "Keine ContentID verfügbar. Bitte geben Sie eine an oder laden Sie zuerst eine Tabelle.",
    );
    return;
  }
...
function setupEventHandlers(contentId) {
  ...
  $(`#${contentId} .sortable`)
    ...
    .on("click", function () {
      ...
      if (instance.state.sort === column) {
        instance.state.sortDir =
          instance.state.sortDir === "ASC" ? "DESC" : "ASC";
      } else {
  ...
  $(`#${contentId} .ui.dropdown[id^="filter_${contentId}_"]`).each(function () {
    ...
    const fullFilterName = $dropdown
      .attr("id")
      .replace(`filter_${contentId}_`, "");
    ...
    $dropdown.dropdown({
      ...
      onChange: function (value, text, $selectedItem) {
        console.log(
          `Dropdown geändert: ${filterName}, Wert: ${value}, Original Filter: ${fullFilterName}`,
        );
    ...
    if (instance.state.filters[fullFilterName]) {
      $dropdown.dropdown(
        "set selected",
        instance.state.filters[fullFilterName],
      );
    }
  });

  // Modal-Trigger einrichten
  $(`#${contentId} [data-modal]`)
    .off("click")
    .on("click", function () {
      const modalId = $(this).data("modal");
      const $modal = $("#" + modalId);
      ...
      $.ajax({
        ...
        error: function () {
          $modal
            .find(".content")
            .html(
              "<div class='ui negative message'>Fehler beim Laden des Inhalts.</div>",
            );
          $modal.modal("show");
        },
      });
  ...
  $(`#clear_filters_${contentId}`)
    .off("click")
    .on("click", function () {
      console.log(`Clearing all filters for ${contentId}`);
      // Clear all filter values in state
      instance.state.filters = {};
      instance.state.search = "";
      instance.state.page = 1;
      // Clear search input
      $(`#search_${contentId}`).val("");
      // Clear all filter dropdowns
      $(`#${contentId} .ui.dropdown[id^="filter_${contentId}_"]`).each(
        function () {
          $(this).dropdown("clear");
        },
      );
...
function setupAutoReload(contentId) {
  ...
  if (interval > 0) {
    console.log(
      `Auto-Reload für ${contentId} mit Intervall ${interval}ms eingerichtet`,
    );
